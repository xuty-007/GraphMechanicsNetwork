...
from n_body_system.dataset_nbody import NewNBodyMStickDataset
...
parser.add_argument('--max_edge', type=int, default=5,
                    help='Max number of edges to sample for length preservation.')
...
if args.config_by_file:
    job_param_path = 'simple_config_md17.json'
...
def main():
    ...
    dataset_train = MD17Dataset(partition='train', max_samples=args.max_training_samples, data_dir=args.data_dir,
                                molecule_type=args.mol, max_conf_edge=args.max_edge, delta_frame=args.delta_frame)
    ...
    if args.model == 'gnn':
        ...
        model = EGNN_vel(in_node_nf=1, in_edge_nf=2 + 3, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
                         recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
    elif args.model == 'egnn_vel_cons':
        model = EGNN_vel(in_node_nf=1, in_edge_nf=2 + 3, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
                         recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
    elif args.model == 'gmn':
        model = GMN(in_node_nf=1, in_edge_nf=2 + 3, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
                    recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
